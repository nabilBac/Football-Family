 ...
 * controller = new VideoApiController(commentService, videoService,
 * userService);
 * mockMvc = MockMvcBuilders.standaloneSetup(controller)
 * .setControllerAdvice(new RestExceptionHandler()) // Ajout de l'exception
 * handler
 * .build();
 * }
 * 
 ...
 * Page<Comment> commentPage = new PageImpl<>(comments, PageRequest.of(0, 5),
 * 2);
 * 
 * when(commentService.getCommentsForVideo(videoId, 0,
 * 5)).thenReturn(commentPage);
 * 
 ...
 * 
 * when(commentService.addComment(eq(videoId), eq("Super vidéo !"),
 * eq("testUser"))).thenReturn(commentDto);
 * 
 ...
 * assertEquals("Le contenu du commentaire ne peut pas être vide.",
 * ex.getMessage());
 * });
 * }
 * 
